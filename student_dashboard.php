...
$stmt_current_credits->bind_param("i", $student_id);
...
// We will consider a student to have passed a semester if they have at least one subject 
// with marks and have passed more than 50% of their enrolled subjects
$sql_passed_semester = "SELECT COUNT(*) as total_subjects,
                        SUM(CASE WHEN 
                            (marks >= 40 OR (marks IS NULL AND lab_marks >= 40)) 
                            THEN 1 ELSE 0 END) as passed_subjects
                        FROM student_subjects ss
                        LEFT JOIN student_marks sm ON ss.student_id = sm.student_id AND ss.subject_id = sm.subject_id
                        WHERE ss.student_id = ?";
                        
$stmt_passed_semester = $conn->prepare($sql_passed_semester);
$stmt_passed_semester->bind_param("i", $student_id);
...
// If student has passed the semester, only show failed subjects
if ($has_passed_semester) {
    // Query to get subjects the student has failed
    $sql_chosen = "SELECT s.* FROM subjects s
                  JOIN student_subjects ss ON s.id = ss.subject_id
                  LEFT JOIN student_marks sm ON ss.student_id = sm.student_id AND s.id = sm.subject_id
                  WHERE ss.student_id = ? AND (sm.marks < 40 OR (sm.marks IS NULL AND sm.lab_marks < 40))";
} else {
    // Normal query for all chosen subjects
...
$stmt_chosen->bind_param("i", $student_id);
$stmt_chosen->execute();
...
<body>
    <div class="container mt-5">
        ...
        <div class="semester-passed-alert">
            <i class="bi bi-check-circle-fill"></i> You have passed this semester! Only failed subjects and subjects you haven't chosen yet are shown below.
        </div>
        <?php endif; ?>
        ...
        <div class="subject-table">
            <h3><?php echo $has_passed_semester ? 'Failed Subjects' : 'Your Selected Subjects'; ?></h3>
                        ...
                        <?php if ($result_chosen->num_rows > 0): ?>
                            <?php while ($row = $result_chosen->fetch_assoc()): ?>
                                ...
                                if ($has_passed_semester) {
                                    ...
                                    $stmt_marks->bind_param("ii", $student_id, $row['id']);
                                    $stmt_marks->execute();
